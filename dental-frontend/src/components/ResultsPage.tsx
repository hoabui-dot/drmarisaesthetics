...
const ALL_PROCEDURES_FILTER_KEY = '__all_procedures__'
...
export function ResultsPage({ data }: { data: ResultsData }) {
  ...
  const [filter, setFilter] = useState<string>(ALL_PROCEDURES_FILTER_KEY); const [selectedCase, setSelectedCase] = useState<ResultCase | null>(null); const lastTriggerRef = useRef<HTMLElement | null>(null)
  const [batchSize, setBatchSize] = useState(6); const [visibleCount, setVisibleCount] = useState(6)
  const categories = data.categories
  const filteredCases = useMemo(() => filter === ALL_PROCEDURES_FILTER_KEY ? data.cases : data.cases.filter((item) => {
    const legacyCategoryId = item.categoryId || data.categories.find((category) => category.label.toLocaleLowerCase() === item.category?.toLocaleLowerCase())?.id
    const categoryIds = item.categoryIds?.length ? item.categoryIds : legacyCategoryId ? [legacyCategoryId] : []
    return categoryIds.includes(filter)
  }), [data.cases, data.categories, filter])
  const visibleCases = filteredCases.slice(0, visibleCount)
  const renderFilterButton = (id: string, label: string) => (
    <button key={id} type="button" className={filter === id ? 'is-active' : ''} aria-pressed={filter === id} onClick={() => setFilter(id)}>
      {filter === id && <motion.span layoutId="results-active-filter" className="results-filters__active-surface" transition={reduceMotion ? { duration: 0 } : { duration: .3, ease: 'easeOut' }} />}
      <span className="results-filters__label">{label}</span>
    </button>
  )
