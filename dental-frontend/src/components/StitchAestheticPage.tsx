...
export function StitchAboutUs({ heroImage, content }: { heroImage?: string | null; content?: any } = {}) {
  ...
  const heroPrimaryButtonLabel = cmsHero.primaryButtonLabel || 'MEET DR. MARIS'
  const heroSecondaryButtonLabel = cmsHero.secondaryButtonLabel || 'REQUEST AN ONLINE CONSULTATION'
  const heroSecondaryButtonLink = cmsHero.secondaryButtonLink || '/contact'
  ...
  return <div ref={aboutRef} className="stitch-page stitch-about">
      ...
      <div className="stitch-container stitch-grid stitch-grid--two">
        <div>
          ...
          <div className="stitch-actions" data-team-hero-actions>
            <a className="stitch-button stitch-button--dark" href="/team">{heroPrimaryButtonLabel}</a>
            <a className="stitch-button stitch-button--outline" href={heroSecondaryButtonLink}>{heroSecondaryButtonLabel}</a>
          </div>
        </div>
    ...
    <section className="stitch-section stitch-consultation"><div className="stitch-container"><div><span className="stitch-kicker">{cmsConsultation.eyebrow || 'TAKE THE FIRST STEP'}</span><h2>{cmsConsultation.title || 'Begin Your Personalized Aesthetic Journey'}</h2></div><div className="stitch-actions"><ConsultationButton>{cmsConsultation.button_label || 'BOOK A CONSULTATION'}</ConsultationButton><a className="stitch-button stitch-button--outline" href={cmsConsultation.secondary_link || '/services'}>{cmsConsultation.secondary_label || 'EXPLORE OUR SERVICES'}</a></div></div></section>
