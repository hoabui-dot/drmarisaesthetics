...
export function StitchHomepage({ data }: { data?: HomepageEditorialData }) {
  ...
  const heroTitle = typeof hero.title === 'string' && hero.title.trim()
    ? hero.title.trim()
    : Array.isArray(hero.title_lines) && hero.title_lines.length
      ? hero.title_lines.map(String).filter(Boolean).join(' ')
      : [hero.title_line_1, hero.title_line_2].filter((value): value is string => typeof value === 'string' && value.trim().length > 0).join(' ') || 'Plastic Surgery in Vietnam for International Patients'
...
export function StitchAboutUs({ heroImage, content }: { heroImage?: string | null; content?: any } = {}) {
  ...
  const heroTitle = cmsHero.headingPrimary || 'About Us'
  const heroLead = cmsHero.supportingParagraph || 'Cosmetic surgery should begin with a medical assessment, a clear surgical plan and confidence in the surgeon responsible for your care.'
