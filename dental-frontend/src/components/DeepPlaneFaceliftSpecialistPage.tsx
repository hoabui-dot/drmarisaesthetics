...
import { ArrowRight, CheckCircle2, Clock3, MapPin, PhoneCall, ShieldCheck, X } from 'lucide-react'
    ...
    <section className="deep-plane-facelift__hero" style={sectionStyle('hero')}>
      <div className="stitch-container deep-plane-facelift__hero-grid">
        <div className="deep-plane-facelift__hero-copy">
          ...
          <figure className="deep-plane-facelift__hero-image deep-plane-facelift__hero-image--mobile" aria-hidden="true"><Image src={page.hero.image} alt="" fill priority sizes="(max-width: 767px) 100vw, 1px" unoptimized /></figure>
        ...
        <figure className="deep-plane-facelift__hero-image deep-plane-facelift__hero-image--desktop"><Image src={page.hero.image} alt={page.hero.imageAlt} fill priority sizes="(max-width: 900px) 100vw, 42vw" unoptimized /></figure>
      </div>
    </section>
...
function CertificationCard({ image, imageAlt, imageLabel, imageIssuer }: { image: string; imageAlt: string; imageLabel?: string; imageIssuer?: string }) {
  ...
  return <>
    <button
      type="button"
      className="deep-plane-facelift__certificate deep-plane-facelift__certificate--image deep-plane-facelift__certificate-trigger"
      aria-label={`View full certificate image${imageAlt ? `: ${imageAlt}` : ''}`}
      onClick={() => dialogRef.current?.showModal()}
    >
      <Image src={image} alt={imageAlt} fill sizes="(max-width: 900px) 86vw, 40vw" unoptimized />
      {(imageLabel || imageIssuer) && <span className="deep-plane-facelift__certificate-image-bottom">
        {imageLabel && <span className="deep-plane-facelift__certificate-image-label">{imageLabel}</span>}
        {imageIssuer && <span className="deep-plane-facelift__certificate-image-issuer">{imageIssuer}</span>}
      </span>}
    </button>
    <dialog
      ref={dialogRef}
      className="deep-plane-facelift__certificate-lightbox"
      aria-label={imageAlt || 'Certificate image'}
      onClick={closeOnBackdrop}
    >
      <button
        type="button"
        className="deep-plane-facelift__certificate-lightbox-close"
      ...
      <Image
        ...
        sizes="100vw"
        unoptimized
        className="deep-plane-facelift__certificate-lightbox-image"
      />
    </dialog>
  </>
}
