...
export function ContactConsultationSection({ data }: { data: ContactConsultationData }) {
  ...
  const [form, setForm] = useState({ name: '', phone: '', email: '', service: services[0]?.value || '', location: locations[0]?.value || '', message: '', consent: false })
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ fullName: form.name.trim(), phoneNumber: form.phone.trim(), email: form.email.trim(), service: form.service, message: `${form.location}: ${form.message.trim()}`, recaptchaToken }),
      })
      if (!response.ok) throw new Error('Contact request failed')
      setStatus('success')
      setForm({ name: '', phone: '', email: '', service: services[0]?.value || '', location: locations[0]?.value || '', message: '', consent: false })
    } catch {
      setStatus('error')
    }
  }
  ...
  return (
    <section id="contact-consultation" className="contact-consultation-section" aria-labelledby="contact-consultation-title">
      <div className="contact-consultation-container">
        <div className="contact-consultation-form-card">
          ...
          <form className="contact-consultation-form" onSubmit={submit} noValidate>
            ...
            <CheckBoxBase checked={form.consent} onChange={(checked) => update('consent', checked)} required><span>Tôi đồng ý với {data.privacyPolicyHref ? <Link href={data.privacyPolicyHref}>{data.privacyPolicyLabel}</Link> : data.privacyPolicyLabel} của Smilux Dental</span></CheckBoxBase>
        ...
        <aside className="contact-consultation-info" aria-labelledby="contact-consultation-info-title">
          ...
          <div className="contact-consultation-advisor">
            {data.advisorImage ? <Image src={data.advisorImage} alt="Bác sĩ tư vấn nha khoa Smilux" width={76} height={76} className="contact-consultation-advisor-image" /> : null}
