...
export function DoctorSection({ data }: { data: HomepageDoctorBlock }) {
  ...
  return <section id="home-doctors" className="doctor-team-section" aria-labelledby="home-doctors-heading"><div className="mx-auto max-w-home-container px-4 py-16 sm:px-6 lg:px-8 lg:py-20"><header className="doctor-team-header"><div><p className="eyebrow">{data.eyebrow || 'OUR DENTIST TEAM'}</p><h2 id="home-doctors-heading">{data.title || 'Meet Our Expert Dentists'}</h2></div><Link href={data.viewAllLink || '/doctors'} className="doctor-view-all">{data.viewAllLabel || 'VIEW ALL DOCTORS'} <ArrowRight aria-hidden="true" /></Link></header><div className="doctor-card-group" data-count={doctors.length}><FeaturedDoctorCard doctor={featured} />{standardDoctors.map((doctor) => <StandardDoctorCard key={doctor.id} doctor={doctor} />)}</div></div></section>
}
