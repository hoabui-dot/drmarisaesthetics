...
export function HeroShell({ id, headingId, backgroundImage, children, bottomContent }: HeroShellProps) {
  return (
    <section
      ...
      className="home-hero-shell relative isolate min-h-screen overflow-hidden rounded-br-hero bg-surface-subtle"
      data-parity-section="hero"
    >
      {backgroundImage && (
        <div className="home-hero-media absolute inset-0 z-0" aria-hidden="true">
          <Image src={typeof backgroundImage === 'string' ? backgroundImage : backgroundImage.url} alt="" fill priority sizes="(max-width: 767px) 150vw, 100vw" className="object-cover" />
        </div>
      )}
      <div className="home-hero-overlay pointer-events-none absolute inset-0 z-10" aria-hidden="true" />
