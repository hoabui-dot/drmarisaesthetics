...
export function OurTeamPage({ data = ourTeamMockData }: { data?: OurTeamData }) {
  ...
  return <main ref={pageRef} className="stitch-page stitch-our-team">
    ...
    <section id="revision" className="our-team-revision" data-team-revision><div className="stitch-container our-team-revision__grid"><div className="our-team-revision__copy"><span className="stitch-kicker" data-revision-reveal>{revision.eyebrow}</span><h2 data-revision-reveal>{revision.title}</h2><div className="our-team-revision__media our-team-revision__media--mobile" aria-hidden="true"><TeamImage src={revision.image || hero.image} alt="" /></div><p data-revision-reveal>{revision.description}</p><div className="our-team-revision__concerns">{(revision.concerns.length ? revision.concerns : revisionConcerns.map(title => ({ title, description: '' }))).map(item => <span key={item.title} data-revision-reveal><ArrowRight size={14} aria-hidden="true" />{item.title}</span>)}</div><p className="our-team-revision__note" data-revision-reveal>{revision.calloutDescription}</p><div data-revision-reveal><ConsultationButton>Request a Revision Surgery Assessment</ConsultationButton></div></div><div className="our-team-revision__media our-team-revision__media--desktop" data-revision-media><TeamImage src={revision.image || hero.image} alt={revision.imageAlt || 'Specialist surgeon examining medical scans for revision planning'} /></div></div></section>

    <section className="our-team-hospital"><div className="stitch-container our-team-hospital__grid"><div className="our-team-hospital__media our-team-hospital__media--desktop"><TeamImage src={data.hospital.image || data.hospitalImage || hero.image} alt={data.hospital.imageAlt || 'Modern surgical theater at City International Hospital'} /></div><div className="our-team-hospital__copy"><Hospital size={34} aria-hidden="true" /><span className="stitch-kicker">{data.hospital.eyebrow}</span><h2>{data.hospital.title}</h2><div className="our-team-hospital__media our-team-hospital__media--mobile" aria-hidden="true"><TeamImage src={data.hospital.image || data.hospitalImage || hero.image} alt="" /></div><p>{data.hospital.description}</p><ul>{data.hospital.proofItems.map(item => <li key={item}><CheckCircle2 size={16} aria-hidden="true" />{item}</li>)}</ul></div></div></section>

    <section className="our-team-international"><div className="stitch-container our-team-international__grid"><div className="our-team-international__copy"><span className="stitch-kicker">{international.eyebrow}</span><h2>{international.title}</h2><div className="our-team-international__media our-team-international__media--mobile" aria-hidden="true"><TeamImage src={data.internationalImage || hero.image} alt="" className="our-team-international__image" /></div><p className="stitch-lead">{international.description}</p><p className="our-team-review-label">What you can provide for preliminary review:</p><ul>{(international.steps.length ? international.steps : reviewItems.map(title => ({ title }))).map(item => <li key={item.title}><CheckCircle2 size={16} aria-hidden="true" />{item.title}</li>)}</ul><blockquote>&quot;Your trip should begin with information — not uncertainty.&quot;</blockquote><p className="our-team-international__note">Note: Remote consultation supports preliminary planning but does not replace the in-person examination required before a final surgical decision.</p><div className="stitch-actions"><ConsultationButton>Request an Online Consultation</ConsultationButton><a className="stitch-link" href="#consultation">International Patient Guide <ArrowRight size={17} /></a></div></div><div className="our-team-international__media our-team-international__media--desktop"><TeamImage src={data.internationalImage || hero.image} alt="Dr. Maris conducting a remote patient consultation" className="our-team-international__image" /></div></div></section>
