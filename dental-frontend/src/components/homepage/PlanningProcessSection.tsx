...
export function PlanningProcessSection({
  ...
  useEffect(() => {
    ...
    const stepObservers = stepElements.map((step, index) => onScroll({
      target: step,
      enter: '62% top',
      leave: '38% bottom',
      repeat: true,
      onEnter: () => {
        if (reduceMotion || index === activeIndexRef.current) return
        activeIndexRef.current = index
        setActiveIndex(index)
        frameRef.current = window.requestAnimationFrame(() => playStep(index))
      },
      onEnterBackward: () => {
        if (reduceMotion || index === activeIndexRef.current) return
        activeIndexRef.current = index
    ...
    return () => {
      ...
      stepObservers.forEach((observer) => observer.revert())
      timelinesRef.current.forEach((timeline) => timeline.pause().revert())
