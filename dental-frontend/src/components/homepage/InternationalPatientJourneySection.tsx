...
export function InternationalPatientJourneySection() {
  ...
  return <section ref={ref} id="journey" className="stitch-section stitch-surface stitch-journey" data-journey-module>
    <div className="stitch-journey-pin" data-journey-pin><div className="stitch-journey-intro"><span className="stitch-journey-intro__eyebrow">The Process</span><h2 data-journey-intro>Your International Patient Journey</h2><p className="stitch-journey-intro__subtitle">A seamless, medically-supervised experience from your first inquiry to your final recovery.</p></div><div className="stitch-journey-layout"><aside className="stitch-journey-indicator" aria-label="International patient journey progress"><div className="stitch-journey-indicator__rail"><span className="stitch-journey-indicator__track" aria-hidden="true" /><span className="stitch-journey-indicator__progress" data-journey-progress aria-hidden="true" />{steps.map(([title], index) => <span className="stitch-journey-indicator__step" data-journey-indicator key={title}><b>0{index + 1}</b><span>{title}</span></span>)}</div></aside><div className="stitch-journey-content"><div className="stitch-journey-copy">{steps.map(([title, copy, image, alt], index) => <article key={title} data-journey-step><span className="stitch-journey-number" aria-hidden="true">0{index + 1}</span><div className="stitch-journey-mobile-image"><Image src={image} alt={alt} fill sizes="100vw" className="object-cover" unoptimized /></div><h3>{title}</h3><p>{copy}</p></article>)}</div></div><div className="stitch-journey-visual"><div className="stitch-journey-visual__frame">{steps.map(([title, , image, alt]) => <div className="stitch-journey-visual__image" data-journey-visual key={title}><Image src={image} alt={alt} fill sizes="(max-width: 900px) 100vw, 48vw" className="object-cover" unoptimized /></div>)}</div><span className="stitch-journey-visual__caption">DR. MARIS AESTHETICS / HO CHI MINH CITY</span></div></div></div>
  </section>
}
