    ...
    <section className="knowledge-center-hero" aria-labelledby="knowledge-center-title">
      <div className="knowledge-center-container">
        ...
        <div className="knowledge-center-hero-grid">
          <div><h1 id="knowledge-center-title">Knowledge Center</h1><p>Expert dental knowledge from Smilux to help you understand, prevent, and care for your smile.</p></div>
    ...
    <section className="knowledge-center-latest knowledge-center-container" aria-labelledby="latest-articles-title">
      <div className="knowledge-center-latest-layout">
        ...
        <aside className="knowledge-center-sidebar">
          ...
          <div className="knowledge-center-question"><div><h2>Have a Question?</h2><p>Our dental experts are here to help you with your concerns.</p><button type="button" onClick={openBookingModal}>Book Consultation <ArrowRight aria-hidden="true" /></button></div><div className="knowledge-center-doctors" aria-hidden="true"><Image src="/api/strapi-media/uploads/doctor_new_4622af55c6.jpg" alt="" fill sizes="150px" /><Image src="/api/strapi-media/uploads/michelle_jin_07462af8b2.jpg" alt="" fill sizes="150px" /></div></div>
        </aside>
      </div>
    </section>
    <section className="knowledge-center-newsletter knowledge-center-container" aria-labelledby="newsletter-title"><div className="knowledge-center-newsletter-copy"><span className="knowledge-center-mail-icon"><Mail aria-hidden="true" /></span><div><h2 id="newsletter-title">Stay Informed, Stay Healthy</h2><p>Subscribe to our newsletter for the latest dental tips, treatment updates, and exclusive offers.</p></div></div><form className="knowledge-center-newsletter-form" onSubmit={(event) => { event.preventDefault(); if (!email.includes('@')) { setNewsletterStatus('Please enter a valid email address.'); return; } setNewsletterStatus('Thank you for subscribing.'); setEmail(''); }}><label className="sr-only" htmlFor="newsletter-email">Email address</label><input id="newsletter-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Enter your email" required /><button type="submit">Subscribe <Send aria-hidden="true" /></button>{newsletterStatus ? <span role="status">{newsletterStatus}</span> : null}</form></section>
  </div>;
}
